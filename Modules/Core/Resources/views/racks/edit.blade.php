...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form method="POST" action="{{ route('racks.update', $rack) }}" class="space-y-4">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select name="outlet_id" required
                    ...
                    <option value="{{ $outlet->id }}" {{ old('outlet_id', $rack->outlet_id) == $outlet->id ? 'selected' : '' }}>{{ $outlet->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $rack->name) }}" required
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi / Keterangan</label>
                <input type="text" name="location_note" value="{{ old('location_note', $rack->location_note) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">{{ old('description', $rack->description) }}</textarea>
            </div>
